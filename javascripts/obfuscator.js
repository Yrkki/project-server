...
async function obfuscateWhenNeeded(currentPath, data) {
    console.debug('obfuscator.js: obfuscateWhenNeeded: Obfuscating...');

    // Dynamically import ES Module when running in a CommonJS context
    const urlExists = (await import('url-exist')).default;

    const a = 'auth.json';
    if (currentPath.startsWith('http')) {
        const authUrl = currentPath + '/' + a;
        console.debug('obfuscator.js: obfuscateWhenNeeded: Checking url:', authUrl);

        // Directly await the Promise-based function
        const exists = await urlExists(authUrl);
        return exists ? data : obfuscate(data);
    } else {
        const authPath = path.resolve(currentPath, a);
        console.debug('obfuscator.js: obfuscateWhenNeeded: Checking path:', authPath);
